    ...
    for y in range(len(map)):
        for x in range(len(map[y])):
            if isinstance(map[y][x], int):
                cost_map[(x, y)] = map[y][x]


    max_x, max_y = 0, 0
    for x, y in cost_map.keys():
        max_x = max(max_x, x)
        max_y = max(max_y, y)

    max_len = 0
    for cost in cost_map.values():
        max_len = max(max_len, len(str(cost)))

    rows = [[" " * max_len for _ in range(max_x + 1)] for _ in range(max_y + 1)]

    for node, cost in cost_map.items():
        rows[node[1]][node[0]] = str(cost).rjust(max_len, "0")
...
def read_from_file():
    with open("./day_16/test.txt", 'r') as file:
